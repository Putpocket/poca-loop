...
class UserHave(Base):
    ...
    photocard_id: Mapped[int] = mapped_column(
        ForeignKey("photocards.id", ondelete="CASCADE"), nullable=False
    )
    ...
    photocard: Mapped["Photocard"] = relationship()
    condition_grade: Mapped[ConditionGrade] = relationship()


class UserWant(Base):
    __tablename__ = "user_wants"
    __table_args__ = (UniqueConstraint("user_id", "photocard_id", name="uq_want_user_card"),)
