...
- 1:1 직접 교환 후보 조회
- 3자 순환 교환 후보 조회
- 로그인 사용자 전용 정식 포카 Have/Want 탐색 목록
...
6. 사용자는 로그인 후 탐색 목록에서 다른 사용자의 정식 포카 Have/Want를 읽기 전용으로 확인합니다.
7. 사용자는 매칭 화면에서 교환 제안 문구를 복사해 외부 채팅에 붙여넣습니다.
8. 사용자는 본인 Have/Want 목록을 공유용 체크리스트로 미리보고 SVG 또는 PNG 파일로 다운로드합니다.
9. 다운로드한 이미지를 사용자가 선택한 SNS나 외부 채팅에 직접 올립니다.
...
- 교환 탐색 목록은 로그인 사용자만 조회할 수 있으며 email, 권한, 토큰, 내부 보안 필드를 반환하지 않습니다.
- 교환 탐색 목록은 정식 `photocard_id` 기반 Have/Want만 노출하며, 임시 포카는 승인 또는 병합되어 정식 포카로 이전되기 전까지 노출하지 않습니다.
...
2. Have/Want를 등록합니다.
3. 로그인 전용 교환 탐색 목록에서 정식 포카 Have/Want를 확인합니다.
4. 1:1 매칭과 3자 매칭을 확인합니다.
5. 교환 제안 문구를 복사합니다.
6. 공유용 체크리스트를 미리보고 SVG 또는 PNG 파일로 다운로드합니다.
7. 임시 포카를 등록합니다.
8. 관리자 화면에서 임시 포카를 승인 또는 병합합니다.
9. 사용자 목록에서 임시 배지가 사라지고 정식 포카로 표시되는지 확인합니다.
