...
export const conditionGuide = [
  ...
  { code: "D", short: "심한 손상", description: "접힘, 오염, 큰 찍힘, 물결, 심한 손상 있음" }
];
      ...
      <div className="grid gap-2 rounded-md border border-slate-200 bg-slate-50 p-3">
        ...
        <div className="grid gap-2 sm:grid-cols-5">
          {conditionGuide.map((item) => (
            <div key={item.code} className="rounded-md border border-slate-200 bg-white px-2 py-2">
              <Badge>{item.code}</Badge>
              <p className="mt-1 text-xs font-medium text-slate-700">{item.short}</p>
            </div>
          ))}
        </div>
      </div>
